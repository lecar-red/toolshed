...
#
# heavily borrowed/hacked from: https://github.com/Xeoncross/lowendscript
#
...
#
# dotdeb is a great place that maintains very up to do
# debian packages on useful stuff like nginx, mysql + percona and redis
#
...
# TODO: add function to enable innodb
...
# TODO: add install of 'build-essential'
function install_cpanm {
	if [ -f /opt/cpanm ]
   	then
		print_warn "cpanm is already installed";	
		return
	fi

	wget https://raw.github.com/miyagawa/cpanminus/master/cpanm -O /opt/cpanm

	chmod 755 /opt/cpanm
	ln -s /opt/cpanm /usr/bin/

	print_info "cpanm has been successfully installed"
}

#
# TODO: consider better name
# thanks to:
# http://library.linode.com/web-servers/nginx/perl-fastcgi/debian-6-squeeze 
#
function install_perl_fcgi {

	if [ -z "$1" -o -z "$2" ]
	then
		die "Usage: `basename $0` perl_fcgi [appdir] [domain name]"
	fi

	check_install nginx nginx

	DEBIAN_FRONTEND=noninteractive apt-get -y install fcgiwrap spawn-fcgi

	# assume that user has moved app into spot
	#mkdir -p /var/www/$1/public
	mkdir -p /var/www/$1/logs

	# give perms to log dir only might need to do this with public
...
server {
	...
	access_log  /var/www/$1/logs/nginx-access.log;
	error_log  /var/www/$1/logs/nginx-error.log;
	...
	# static content, serve from nginx

	location / {
        gzip off;
        include /etc/nginx/fastcgi_params;
        fastcgi_pass unix:/var/run/fcgi-app.socket;
        fastcgi_param SCRIPT_FILENAME /var/www/$1/public\$fastcgi_script_name/\$1;
    }
}
END
	...
	print_warn "perl_fastcgi for dancer setup"
}

#
# TODO: add starman setup and install + daemon control/tools
#

# install and configure perl dancer
function install_perl_dancer {
	if [ -z "$1" -o -z "$2" ]
	then
		die "Usage: `basename $0` perl_dancer [appdir] [domain name]"
	fi
	...
	# not sure how to handle this, maybe add 'force' option
	#if [ -f "/etc/nginx/sites-available/$2.conf" ];
		#print_warn "Site '$2' configured and installed";
		#return
	#fi

	# install needed perl modules
	cpanm Dancer YAML Plack Starman

	mkdir -p $1/logs
	chmod 777 $1/logs

	# not sure on this front 
	chown -R www-data:www-data $1/logs

	document_root=$1/public
...
server {
	...
	access_log  $1/logs/nginx-access.log;
	error_log   $1/logs/nginx-error.log;
	...
	# can't figure out why \$document_root isn't working here
	# but need to spend some time to with nginx docs

	# serve static content with nginx
	location /images {
		#root \$document_root/images;
		root $document_root/images;
	}

	location /css {
		#root \$document_root/css;
		root $document_root/css;
	}

	location /javascripts {
		#root \$document_root/javascripts
		root $document_root/javascripts
	}

	location /bootstrap {
		#root \$document_root/bootstrap
		root $document_root/bootstrap
	}
...
END
	...
	if [ ! -f "/etc/init.d/dancer" ]
	then
		# retrieve awesome dancer init script
		wget -q -O /etc/init.d/dancer https://raw.github.com/bigpresh/misc-scripts/master/dancer-init.d/dancer

		chmod 755 /etc/init.d/dancer

		update-rc.d dancer defaults
	fi

	print_warn "perl_dancer setup"
}

#
# APP SETUP - pulled from bigpresh's debian startup dancer stuff
#
function setup_dancer_app {
	if [ -z "$1" -o -z "$2" ]
	then
		die "Usage: `basename $0` $2 [dns name] [user]"
	fi

	invoke-rc.d dancer stop

	piddir=/var/run/dancer-$2
	user=$1
	
	# these are a bit app dependent
	if [ ! -d /etc/dancer ]
	then
		mkdir /etc/dancer
	fi
	...
	if [ ! -f "/etc/dancer/$2" ]
	then
		cat > /etc/dancer/$2 <<END
DIR=/home/$user
SCRIPT=bin/app.pl
USER=$user
PORT=3000
PIDFILE=$piddir/site.pid
STARMAN_OPTS="--error-log=/home/$user/logs/error-log"
END
	fi
	...
	invoke-rc.d dancer start binder

	print_warn "$2 - setup and started"
}
...
function install_mongo_db {
	tengen="/etc/apt/sources.list.d/10gen.list"
	
	# add magic mongodb key	
	key=`DEBIAN_FRONTEND=noninteractive apt-key list |grep 7F0CEB10`

	# no luck add key to ring 
	if [ $? == 1 ]
   	then
		DEBIAN_FRONTEND=noninteractive apt-key adv --keyserver keyserver.ubuntu.com --recv 7F0CEB10
	fi

	# 
	# add mongodb to apt src list if this doesn't exist
	#
	if [ ! -f "$tengen" ]
	then
		echo 'deb http://downloads-distro.mongodb.org/repo/debian-sysvinit dist 10gen' > $tengen
	fi

	# update after adding mongodb to dist
	DEBIAN_FRONTEND=noninteractive apt-get -q -y update

	check_install mongo mongodb-10gen

	invoke-rc.d mongodb stop

	# TODO: consider adjusting ulimit too

	# use default for now, it is tied to localhost only
...
cpanm)
	...
	;;
perl_fcgi)
	install_perl_fcgi $2 $3
	;;
perl_dancer)
	install_perl_dancer $2 $3
	;;
mongodb)
	install_mongo_db
	;;
...
*)
	...
	echo '  - cpanm                  Install Perls cpanm tool)'
	echo '  - perl_fcgi              (Install fastcgi for Perl with nginx config)'
	echo '  - perl_dancer            (Install Perl dancer)'
	echo '  - perl_starman           (Install Perl Starman with startup scripts)'
